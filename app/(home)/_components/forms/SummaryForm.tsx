...
const SummaryForm = (props: { handleNext: () => void }) => {
  ...
  const GenerateSummaryFromAI = async () => {
    try {
      ...
      // Parse the response and handle the correct structure
      const parsedResponse = JSON.parse(responseText);
      
      // Handle both possible response formats
      let summaries;
      if (parsedResponse.summaries && Array.isArray(parsedResponse.summaries)) {
        // If response has "summaries" array, extract the objects
        summaries = parsedResponse.summaries.reduce((acc: any, item: any) => {
          return { ...acc, ...item };
        }, {});
      } else {
        // If response is directly the object
                ...
                ([experienceType, summary], index) => (
                  <Card
                    ...
                    className="my-4 bg-primary/5 shadow-none
                            border-primary/30
                          "
                    onClick={() => handleSelect(typeof summary === 'string' ? summary : String(summary))}
                  >
                    ...
                    <CardContent className="text-sm">
                      <p>{typeof summary === 'string' ? summary : String(summary)}</p>
                    </CardContent>
                  </Card>
                )
              )}
